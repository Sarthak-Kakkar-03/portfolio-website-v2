  ...
  return (
    <div className='text-matte p-5 flex justify-center items-start'>
        <div className='flex flex-col p-2 md:p-5 bg-card rounded-xl shadow gap-3 max-w-[900px]'>
            <div className=' hover:text-highlight mb-5'><Link to={'/#experience'}>&larr; Back to Experience</Link></div>
            <div className='flex gap-3'>
                ...
                <div className='flex flex-col'>
                    ...
                    <h2 className='text-lg md:text-3xl'>{exp.companyName}</h2>
            ...
            <PageSection title={'Responsibilities'}>
                <ul className='list-disc list-inside'>
                    {exp.responsibilities.map((responsibility, idx) => {return (
            ...
            <PageSection title={'Key Achievements'}>
                <ul className='list-disc list-inside'>
                    {exp.achievements.map((achievement, idx) => {return (
            ...
            <PageSection title={"Technologies Used"}>
                <div className='flex flex-wrap gap-3'>
                    {exp.technologies.map((tech, idx) =>{
                        console.log(tech);
                        return (
                            <div key={idx} className='p-2 bg-surface rounded-xl shadow font-semibold'>{tech}</div>
                        );
                    })}
                </div>
            </PageSection>
...
const PageSection = ({title, children}) => {
    return (
        <div className='flex flex-col gap-2'>
            <div className='border-b-2 border-matte p-2 font-bold md:text-lg'>{title}</div>
            {children}
        </div>
    );
}
